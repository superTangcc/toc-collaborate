...
struct Meta<T, 2, REC> {
    private:
        ...
        inline void init_pp(void) {
            ...
            size_t l_addr, l_promoted_base = 0;
            /* handle the reduced array */
            promoted_array_ = new T[n_segs_ * input_size_[cut_dim_]];
            /* since we are supposed to cut into promoted_array_[] 
             * alternatively on dimensions x and y
             */
            promoted_stride_[1] = input_size_[cut_dim_];
            ...
            for (size_t i = 0; i < input_size_[cut_dim_]; ++i,
                    l_promoted_base += promoted_stride_[0]) {
                l_addr = l_promoted_base;
                for (size_t j = 0; j < n_segs_; 
                        ++j, l_addr += promoted_stride_[1]) {
            ...
            input_2D_algo_ = new T_Input_2D_Algo(op_, promoted_array_, n_segs_, input_size_[cut_dim_]);
...
#else
            /* since it will do the preprocessing along the cut_dim_ */
    ...
    public:
        inline T query(size_t (&lo)[2] /* inclusive */, size_t (&hi)[2] /* exclusive */) {
            ...
            if (begin_range[red_dim_] + 1 == end_range[red_dim_]) {
                ...
                /* we switch to align to the data layout of 
                 * promoted_array_[] 
                 */
                begin_range[1] = begin_range[red_dim_];
                end_range[1] = end_range[red_dim_];
                begin_range[0] = lo[cut_dim_];
                end_range[0] = hi[cut_dim_];
