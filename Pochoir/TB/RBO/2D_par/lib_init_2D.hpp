...
// #pragma omp parallel for
                cilk_for (size_t j = 0; j < input_size_[cut_dim_]; ++j) {
                    /* since the input_1D_algo_ to this initial algo is a 1D
                     * algo, we don't set_dim of it
                     */
                    // input_1D_algo_[i][j].set_stride(pp_red_stride);
        ...
        inline void init_pp_i(size_t _i) {
            ...
            for (size_t i = begin_idx; i < end_idx; ++i) {
                ...
                if (l_begin_range < input_size_[cut_dim_]) {
                    ...
                    for ( ; i < len; --k, ++k1, ++i) {
                        l_pp_addr = k * pp_cut_stride;
                        l_addr = k * cut_stride;
                        l_pp_addr_1 = k1 * pp_cut_stride;
                        l_addr_1 = k1 * cut_stride;
                        for (size_t j = 0; j < input_size_[red_dim_]; ++j,
                            l_pp_addr += pp_red_stride,
                            l_addr += red_stride,
    ...
    public:
        ...
        void print_range(size_t (&lo)[2] /* inclusive */, size_t (&hi)[2] /* exclusive */) {
            ...
            cout << "-----------------------------------------------------------------------\n";
            size_t l_lca = lca(lo[cut_dim_], hi[cut_dim_]), l_level = msb_pos(l_lca) - 1;
