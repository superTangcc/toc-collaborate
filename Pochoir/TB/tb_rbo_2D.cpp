 ...
 * ============================================================================
 *
 *       Filename:  tb_rbo_2D.cpp
 *
 *    Description:  test bench for 2d-dimensional RBO
...
int main (int argc, char * argv[]) {
    ...
    size[1] = (unsigned long)atoi(argv[1]);
    size[0] = (unsigned long)atoi(argv[2]);
    // size[1] = pow2(size[1]);
    // size[0] = pow2(size[0]);
    unsigned long * input_array = new unsigned long[size[1] * size[0]];
    ...
    for (unsigned long j = 0; j < size[1]; ++j) {
        for (unsigned long i = 0; i < size[0]; ++i) {
            ...
            input_array[j * size[0] + i] = (rand() % 8 == 0);
        }
    }
...
#define INF 10000000000
    ...
    RBO_counter_d<N_RANK, 1> rbo(NBITS, input_array, size[1], size[0]);
    gettimeofday(&end, 0);
...
#define TIMES 9111
    ...
    printf("RBO passed, array_size (%lu, %lu)!!\n", size[1], size[0]);
    printf("RBO query takes : %.6f ms, naive scan takes : %.6f ms\n", 
