...
void print_papi_measurements(PAPI_event_set_wrapper_t* event_sets, double* measurements,
         ...
         *                      num_sets, num_threads, num_measurements );
         *
...
/* -----------------------------------------------------------------
 *  PAPI_MAKE_MEASUREMENTS( &set, CODE, results, idx_start, num_trials );
 *
 *  Input  : set, CODE, num_trials, results, index
 *  Output : results
 *
 ...
 *  length >= num_trials.
 */
#define PAPI_MAKE_MEASUREMENTS( set, CODE, results, idx_start, num_trials ) { \
		long_long _counter; \
		int _retval, _i; \
		for( _i = 0; _i < num_trials; _i++ ) { \
			...
			results[idx_start+_i] = (double)_counter; \
		} \
	}
