  ...
  for (block_i=0; block_i < numBlocks_x; block_i++) {
	...
	for (t=0; t < numIter; t++) {
	  ...
	  for (i=blockMin_x; i < blockMax_x; i++) {
		...
		Write_start[Index3D(i,j,k)] = S0 * Read_start[Index3D(i,j,k)] + 
		  S1 * (Read_start[Index3D(i-1,j,k)] + Read_start[Index3D(i+1,j,k)] + Read_start[Index3D(i,j-1,k)] + Read_start[Index3D(i,j+1,k)] + Read_start[Index3D(i,j,k-1)] + Read_start[Index3D(i,j,k+1)]);
	      }
	    }
	  }
	  temp_ptr = Read_start;
