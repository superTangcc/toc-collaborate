...
my $output_file = ("./" . $platform . "_" . $compiler . "_random_3D_" . $nreals_x . "_" . $nreals_y . "_" . $nreals_z . "_" . $numIter . "i.txt");
for (my $i=0; $i < $num_data_points; $i++) {
  ...
  while ($config_pt_in_config_list == 1) {
    ...
    $statement_type = int(rand(1));
    $read_from_type = int(rand(1));
    $pointer_type = int(rand(1));
    $neighbor_index_type = int(rand(1));
    $fma_ins_type = int(rand(1));
    $prefetch_index_type = int(rand(1));
    $prefetch_distance = int(rand(1));
    $rb_size = int(rand(2));
