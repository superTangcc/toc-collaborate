...
@rb_sizes = ([1,1,1], [1,1,2], [1,2,1], [1,2,2], [2,1,1], [2,1,2], [2,2,1], [2,2,2], [1,1,4], [1,2,4], [2,1,4], [1,1,8]);
...
$specified_data_script = ("specified_27pt_cse_coll_thread_data_generator.pl");
...
$greedy_data_script = ("greedy_27pt_cse_coll_thread_data_generator.pl");
...
$random_data_script = ("random_27pt_cse_coll_thread_data_generator.pl");
...
sub init_fixed_ptrs {
	...
	print OUT ("                      int32_t i_" . convert_point_to_string("_", @{$neighbor_read_indices[$i]}) . " = Index3D(i", i2s($neighbor_read_indices[$i][0]-1), ",j", i2s($neighbor_read_indices[$i][1]-1), ",threadBlockMin_z", i2s($neighbor_read_indices[$i][2]-1), ");\n");
    }
...
sub init_moving_ptrs {
    ...
    print OUT ("                      Read_rb_" . convert_point_to_string("_", @{$neighbor_read_indices[0]}) . " = Read_start_unrestrict + Index3D(i" . i2s($offset_from_center[0]) . ",j" . i2s($offset_from_center[1]) . ",threadBlockMin_z". i2s($offset_from_center[2]) . ");\n");
    ...
    print OUT ("                      Write_rb_" . convert_point_to_string("_", @{$neighbor_write_indices[0]}) . " = Write_start_unrestrict + Index3D(i" . i2s($offset_from_center[0]) . ",j" . i2s($offset_from_center[1]) . ",threadBlockMin_z". i2s($offset_from_center[2]) . ");\n");
...
sub singly_nested_loop_code {
    ...
    if ($my_rb_size[2] == 1) {
	print OUT ("                      for (k=threadBlockMin_z; k < threadBlockMax_z; k++) {\n");
    }
    else {
	print OUT ("                      for (k=threadBlockMin_z; k < (threadBlockMax_z-" . ($my_rb_size[2]-1) . "); k+=" . $my_rb_size[2] . ") {\n");
    }
    ...
    if ($clean_up_code) {
	if ($my_rb_size[2] > 1) {
	    print OUT ("                      for (; k < threadBlockMax_z; k++) {\n");
...
sub doubly_nested_loop_code {
    ...
    if ($my_rb_size[1] == 1) {
	print OUT ("                    for (j=threadBlockMin_y; j < threadBlockMax_y; j++) {\n");
    }
    else {
	print OUT ("                    for (j=threadBlockMin_y; j < (threadBlockMax_y-" . ($my_rb_size[1]-1) . "); j+=" . $my_rb_size[1] . ") {\n");
    }
    ...
    if ($clean_up_code) {
	if ($my_rb_size[1] > 1) {
	    print OUT ("                    for (; j < threadBlockMax_y; j++) {\n");
...
sub triply_nested_loop_code {
    if ($rb_size[0] == 1) {
	print OUT ("                  for (i=threadBlockMin_x; i < threadBlockMax_x; i++) {\n");
    }
    else {
	print OUT ("                  for (i=threadBlockMin_x; i < (threadBlockMax_x-" . ($rb_size[0]-1) . "); i+=" . $rb_size[0] . ") {\n");
    }
    ...
    if ($clean_up_code) {
	if ($rb_size[0] > 1) {
	    print OUT ("                  for (; i < threadBlockMax_x; i++) {\n");
...
sub outer_nested_loop_code {
    ...
    print OUT ("      threadBlockMin_x = realMin_x + cb_i * numCoreBlockCells_x\;\n");
    print OUT ("      threadBlockMax_x = threadBlockMin_x + numCoreBlockCells_x\;\n");
    print OUT ("      for (cb_j=0; cb_j < numCoreBlocks_y; cb_j++) {\n");
    print OUT ("        for (cb_k=0; cb_k < numCoreBlocks_z; cb_k++) {\n");
    print OUT ("          if (((cb_n/numCoreBlocksPerChunk) % (numThreads/numThreadBlocksPerCoreBlock)) == (rank/numThreadBlocksPerCoreBlock)) {\n");
    print OUT ("            tb_n = 0;\n");
    print OUT ("            for (tb_j=0; tb_j < numThreadBlocksPerCoreBlock_y; tb_j++) {\n");
    print OUT ("              for (tb_k=0; tb_k < numThreadBlocksPerCoreBlock_z; tb_k++) {\n");
    print OUT ("                if ((rank % numThreadBlocksPerCoreBlock) == tb_n) {\n");
    print OUT ("                  threadBlock_y = cb_j * numThreadBlocksPerCoreBlock_y + tb_j\;\n");
    print OUT ("                  threadBlock_z = cb_k * numThreadBlocksPerCoreBlock_z + tb_k\;\n");
    print OUT ("                  threadBlockMin_y = realMin_y + threadBlock_y * numThreadBlockCells_y\;\n");
    print OUT ("                  threadBlockMax_y = threadBlockMin_y + numThreadBlockCells_y\;\n");
    print OUT ("                  threadBlockMin_z = realMin_z + threadBlock_z * numThreadBlockCells_z\;\n");
    print OUT ("                  threadBlockMax_z = threadBlockMin_z + numThreadBlockCells_z\;\n");
...
sub generate_single_stencil_function {
    ...
    print OUT ("  int32_t threadBlock_y, threadBlock_z\;\n");
    print OUT ("  int32_t threadBlockMin_x, threadBlockMin_y, threadBlockMin_z\;\n");
    print OUT ("  int32_t threadBlockMax_x, threadBlockMax_y, threadBlockMax_z\;\n");
    print OUT ("  int32_t cb_n, cb_i, cb_j, cb_k\;\n");
    print OUT ("  int32_t tb_n, tb_j, tb_k\;\n");
...
sub generate_directives {
    ...
    print OUT "#define COLLABORATIVE_THREADING\n";
...
sub generate_specified_data_script {
    ...
    print OUT ("\@log2_numThreads_values = (3..7)\;\n");
    ...
    print OUT ("\@log2_desiredThreadBlocksPerCoreBlock_values = (3)\;\n");
    print OUT ("\@log2_numThreadBlockCells_y_values = (1..3)\;\n");
    print OUT ("\@log2_numThreadBlockCells_z_values = (3..4)\;\n");
    ...
    print OUT ("\t\t\t\t\tmy \$output_file = (\"./\" . \$platform . \"_\" . \$compiler . \"_specified_coll_thread_\" . \$nreals_x . \"_\" . \$nreals_y . \"_\" . \$nreals_z . \"_\" . \$numIter . \"i_\" . \$numThreads . \"t.txt\");\n");
    print OUT ("\t\t\t\t\tforeach \$padding (\@padding_values) {\n");
    print OUT ("\t\t\t\t\t\tforeach \$log2_numCoreBlockCells_x (\@log2_numCoreBlockCells_x_values) {\n");
    print OUT ("\t\t\t\t\t\t\tmy \$numCoreBlockCells_x = 2**(\$log2_numCoreBlockCells_x); # 4 to \$nreals_x in powers of 2\n");
    print OUT ("\t\t\t\t\t\t\tforeach \$log2_numCoreBlockCells_y (\@log2_numCoreBlockCells_y_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\tmy \$numCoreBlockCells_y = 2**(\$log2_numCoreBlockCells_y); # 4 to \$nreals_y in powers of 2\n");
    ...
    print OUT ("\t\t\t\t\t\t\t\t\tforeach \$log2_desiredThreadBlocksPerCoreBlock (\@log2_desiredThreadBlocksPerCoreBlock_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\tmy \$desiredThreadBlocksPerCoreBlock = 2**(\$log2_desiredThreadBlocksPerCoreBlock);\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\tforeach \$log2_numThreadBlockCells_y (\@log2_numThreadBlockCells_y_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\tmy \$numThreadBlockCells_y = 2**(\$log2_numThreadBlockCells_y);\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\tforeach \$log2_numThreadBlockCells_z (\@log2_numThreadBlockCells_z_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\tmy \$numThreadBlockCells_z = 2**(\$log2_numThreadBlockCells_z);\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\tmy \$log2_threadBlocksPerCoreBlock = (\$log2_numCoreBlockCells_y + \$log2_numCoreBlockCells_z) - (\$log2_numThreadBlockCells_y + \$log2_numThreadBlockCells_z);\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\tif ((\$numThreadBlockCells_y <= \$numCoreBlockCells_y) && (\$numThreadBlockCells_z <= \$numCoreBlockCells_z) && (\$log2_threadBlocksPerCoreBlock == \$log2_desiredThreadBlocksPerCoreBlock)) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\t\t\@log2_numCoreBlocksPerChunk_values = (0..((\$log2_nreals_x + \$log2_nreals_y + \$log2_nreals_z + \$log2_threadBlocksPerCoreBlock) - (\$log2_numCoreBlockCells_x + \$log2_numCoreBlockCells_y + \$log2_numCoreBlockCells_z + \$log2_numThreads)))\;\n");
    ...
    print OUT ("#\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tsystem (\"echo \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$padding \$numCoreBlockCells_x \$numCoreBlockCells_y \$numCoreBlockCells_z \$numCoreBlocksPerChunk \$numThreadBlockCells_y \$numThreadBlockCells_z 0 \$read_from_type \$pointer_type \$neighbor_index_type 0 \$prefetch_index_type \$prefetch_distance \$rb_size >> \$output_file\")\;\n");
...
sub generate_greedy_data_script {
    ...
    print OUT ("\t\$padding_line_number = \$my_best_timing_line_number - 23\;\n\n");
    print OUT ("\twhile (<TXT>) {\n");
    ...
    print OUT ("\tclose(TXT)\;\n");
    print OUT ("}\n\n");
    print OUT ("sub find_best_core_block_and_chunk_size_and_thread_block {\n");
    print OUT ("\tmy(\$my_file_name, \$my_best_timing_line_number) = \@_;\n\n");
    print OUT ("\topen (TXT, \"\$my_file_name\");\n");
    print OUT ("\t\$core_block_line_number = \$my_best_timing_line_number - 21;\n");
    print OUT ("\t\$chunk_size_line_number = \$core_block_line_number + 2;\n\n");
    print OUT ("\t\$thread_block_line_number = \$chunk_size_line_number + 2;\n");
    print OUT ("\twhile (<TXT>) {\n");
    ...
    print OUT ("\t\t}\n");
    print OUT ("\t\tif (\$. == \$thread_block_line_number) {\n");
    print OUT ("\t\t\t\@best_thread_block_dims = split(/,/, \$_)\n");
    print OUT ("\t\t}\n\n");
    print OUT ("\t}\n\n");
    print OUT ("\t\$output_file_line_length = \$.;\n");
    print OUT ("\tclose(TXT);\n");
    ...
    print OUT ("\tmy(\$my_file_name, \$my_best_timing_line_number) = \@_;\n\n");
    print OUT ("\topen (TXT, \"\$my_file_name\");\n");
    print OUT ("\t\$read_from_type_line_number = \$my_best_timing_line_number - 15;\n");
    ...
    print OUT ("\t\$rb_size_and_case_line_number = \$neighbor_index_type_line_number + 6;\n\n");
    print OUT ("\twhile (<TXT>) {\n");
    ...
    print OUT ("\t\t\t\tforeach \$log2_numThreads (\@log2_numThreads_values) {\n");
    print OUT ("\t\t\t\t\tmy \$numThreads = 2**(\$log2_numThreads);\n\n");
    print OUT ("\t\t\t\t\tmy \$output_file = (\"./\" . \$platform . \"_\" . \$compiler . \"_greedy\${opt_sweep_number}_27pt_cse_coll_thread_\" . \$nreals_x . \"_\" . \$nreals_y . \"_\" . \$nreals_z . \"_\" . \$numIter . \"i_\" . \$numThreads . \"t.txt\");\n");
    ...
    print OUT ("\t\t\t\t\tmy \$init_numCoreBlockCells_x = 2**(\$log2_numCoreBlockCells_x_values[0]);\n");
    print OUT ("\t\t\t\t\tmy \$init_numCoreBlockCells_y = 2**(\$log2_numCoreBlockCells_y_values[0]);\n");
    print OUT ("\t\t\t\t\tmy \$init_numCoreBlockCells_z = 2**(\$log2_numCoreBlockCells_z_values[2]);\n");
    print OUT ("\t\t\t\t\tmy \$init_numCoreBlocksPerChunk = 1;\n");
    print OUT ("\t\t\t\t\tmy \$init_numThreadBlockCells_y = 2**(\$log2_numThreadBlockCells_y_values[0]);\n");
    print OUT ("\t\t\t\t\tmy \$init_numThreadBlockCells_z = 2**(\$log2_numThreadBlockCells_z_values[0]);\n");
    print OUT ("\t\t\t\t\tmy \$init_numThreadBlockCells_z = 2**(\$log2_numThreadBlockCells_z_values[0]);\n");
    ...
    foreach $log2_numThreads (3..7) {
	print OUT ("\t\t\t\t\tif (\$log2_numThreads == $log2_numThreads) {\n");
	print OUT ("\t\t\t\t\t\t\$init_numCoreBlockCells_x = 2**(\$log2_numCoreBlockCells_x_values[0];\n");
	print OUT ("\t\t\t\t\t\t\$init_numCoreBlockCells_y = 2**(\$log2_numCoreBlockCells_y_values[0];\n");
	print OUT ("\t\t\t\t\t\t\$init_numCoreBlockCells_z = 2**(\$log2_numCoreBlockCells_z_values[2];\n");
	print OUT ("\t\t\t\t\t\t\$init_numCoreBlocksPerChunk = 1;\n");
	print OUT ("\t\t\t\t\t\t\$init_numThreadBlockCells_y = 2**(\$log2_numThreadBlockCells_y_values[0];\n");
	print OUT ("\t\t\t\t\t\t\$init_numThreadBlockCells_z = 2**(\$log2_numThreadBlockCells_z_values[0];\n");
	print OUT ("\t\t\t\t\t\t\$init_numThreadBlockCells_z = 2**(\$log2_numThreadBlockCells_z_values[0];\n");
    ...
    print OUT ("\t\t\t\t\t\# PADDING\n");
    print OUT ("\t\t\t\t\tforeach \$padding (\@padding_values) {\n");
    print OUT ("\t\t\t\t\t\tsystem (\"\$exec_file \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$padding \$init_numCoreBlockCells_x \$init_numCoreBlockCells_y \$init_numCoreBlockCells_z \$init_numCoreBlocksPerChunk \$init_numThreadBlockCells_y \$init_numThreadBlockCells_z 0 \$init_read_from_type_value \$init_pointer_type_value \$init_neighbor_index_type_value 0 \$init_prefetch_index_type_value \$init_prefetch_distance_value \$init_rb_size_value >> \$output_file\");\n");
    print OUT ("\t\t\t\t\t}\n");
    ...
    print OUT ("\t\t\t\t\t\# CORE BLOCK, CHUNK SIZE, AND THREAD BLOCK\n");
    print OUT ("\t\t\t\t\tforeach \$log2_numCoreBlockCells_x (\@log2_numCoreBlockCells_x_values) {\n");
    print OUT ("\t\t\t\t\t\tmy \$numCoreBlockCells_x = 2**(\$log2_numCoreBlockCells_x); # 4 to \$nreals_x in powers of 2\n");
    print OUT ("\t\t\t\t\t\tforeach \$log2_numCoreBlockCells_y (\@log2_numCoreBlockCells_y_values) {\n");
    print OUT ("\t\t\t\t\t\t\tmy \$numCoreBlockCells_y = 2**(\$log2_numCoreBlockCells_y); # 4 to \$nreals_y in powers of 2\n");
    ...
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\tforeach \$log2_numCoreBlocksPerChunk (\@log2_numCoreBlocksPerChunk_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\t\tmy \$numCoreBlocksPerChunk = 2**(\$log2_numCoreBlocksPerChunk)\;\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\t\tsystem (\"\$exec_file \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$best_padding_dims[2] \$numCoreBlockCells_x \$numCoreBlockCells_y \$numCoreBlockCells_z \$numCoreBlocksPerChunk \$numThreadBlockCells_y \$numThreadBlockCells_z 0 \$init_read_from_type_value \$init_pointer_type_value \$init_neighbor_index_type_value 0 \$init_prefetch_index_type_value \$init_prefetch_distance_value \$init_rb_size_value >> \$output_file\");\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\t\t\t\t}\n");
    ...
    print OUT ("\t\t\t\t\t}\n\n");
    print OUT ("\t\t\t\t\t\$best_timing_line_number = find_best_time_for_opt(\$output_file, \$output_file_line_length);\n");
    print OUT ("\t\t\t\t\tfind_best_core_block_and_chunk_size_and_thread_block(\$output_file, \$best_timing_line_number);\n");
    print OUT ("\t\t\t\t\tsystem (\"echo \\\"BEST CORE BLOCK, CHUNK SIZE AND THREAD BLOCK:\\\" >> \$output_file\");\n");
    ...
    print OUT ("\t\t\t\t\tsystem (\"echo \\\"Thread block: \@best_thread_block_dims[0,1,2]\\\" >> \$output_file\");\n");
    print OUT ("\t\t\t\t\tsystem (\"echo \\\"Time: \$median_times{\$my_best_timing_line_number}\\\" >> \$output_file\");\n\n");
    ...
    print OUT ("\t\t\t\t\t\t\tforeach \$neighbor_index_type (\@neighbor_index_type_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\tforeach \$rb_size (\@rb_size_values) {\n");
    print OUT ("\t\t\t\t\t\t\t\t\tsystem (\"\$exec_file \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$best_padding_dims[2] \$best_core_block_dims[0] \$best_core_block_dims[1] \$best_core_block_dims[2] \$best_chunk_size[0] \$best_thread_block_dims[1] \$best_thread_block_dims[2] 0 \$read_from_type \$pointer_type \$neighbor_index_type 0 \$init_prefetch_index_type_value \$init_prefetch_distance_value \$rb_size >> \$output_file\");\n");
    print OUT ("\t\t\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t}\n");
    ...
    print OUT ("\t\t\t\t\tforeach \$prefetch_index_type (\@prefetch_index_type_values) {\n");
    print OUT ("\t\t\t\t\t\tforeach \$prefetch_distance (\@prefetch_distance_values) {\n");
    print OUT ("\t\t\t\t\t\t\tsystem (\"\$exec_file \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$best_padding_dims[2] \$best_core_block_dims[0] \$best_core_block_dims[1] \$best_core_block_dims[2] \$best_chunk_size[0] \$best_thread_block_dims[1] \$best_thread_block_dims[2] 0 \$best_read_from_type[0] \$best_pointer_type[0] \$best_neighbor_index_type[0] 0 \$prefetch_index_type \$prefetch_distance \$best_rb_case[0] >> \$output_file\");\n");
    print OUT ("\t\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t\t\$best_timing_line_number = find_best_time_for_opt(\$output_file, \$output_file_line_length);\n");
...
sub generate_random_data_script {
    ...
    print OUT ("my \$output_file = (\"./\" . \$platform . \"_\" . \$compiler . \"_random_27pt_cse_coll_thread_\" . \$nreals_x . \"_\" . \$nreals_y . \"_\" . \$nreals_z . \"_\" . \$numIter . \"i.txt\");\n");
    ...
    print OUT ("    \@config_pt = (\$numThreads, \$padding, \$numCoreBlockCells_x, \$numCoreBlockCells_y, \$numCoreBlockCells_z, \$numCoreBlocksPerChunk, \$read_from_type, \$pointer_type, \$neighbor_index_type, \$prefetch_index_type, \$prefetch_distance, \$rb_size)\;\n");
    ...
    print OUT ("\#  system \(\"echo \\\"Start Time\:\\\" >> \$output_file\"\)\;\n");
    print OUT ("\#  system \(\"date >> \$output_file\"\)\;\n");
    print OUT ("  system (\"\$exec_file \$numIter \$nreals_x \$nreals_y \$nreals_z \$numThreads \$padding \$numCoreBlockCells_x \$numCoreBlockCells_y \$numCoreBlockCells_z \$numCoreBlocksPerChunk 0 0 0 \$read_from_type \$pointer_type \$neighbor_index_type 0 \$prefetch_index_type \$prefetch_distance \$rb_size >> \$output_file\"\)\;\n");
    print OUT ("\#  system \(\"echo \\\"End Time\:\\\" >> \$output_file\"\)\;\n");
    print OUT ("\#  system \(\"date >> \$output_file\"\)\;\n");
    print OUT ("\#  system \(\"echo \\\"***************************************\\\" >> \$output_file\"\)\;\n");
    print OUT ("\#  system \(\"echo >> \$output_file\"\)\;\n");
