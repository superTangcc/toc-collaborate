...
void *pthreads_each(void *rank_ptr) {
  ...
  barrier_wait(&my_barrier, rank);
  stencil_per_thread[statement_type][read_from_type][pointer_type][neighbor_index_type][fma_ins_type][prefetch_index_type][prefetch_distance][rb_size](rank);
  barrier_wait(&my_barrier, rank);
  if (rank == 0) {
    printf("\nGRID A AFTER:");
    print_grid(A);
  ...
  for (papi_setnum=0; papi_setnum < num_sets; papi_setnum++) {
    PAPI_MAKE_MEASUREMENTS(event_sets[papi_setnum].set, stencil_per_thread[statement_type][read_from_type][pointer_type][neighbor_index_type][fma_ins_type][prefetch_index_type][prefetch_distance][rb_size](rank), results, NUM_TRIALS*(papi_setnum*numThreads+rank), NUM_TRIALS);
  }
  barrier_wait(&my_barrier, rank);
  if (rank == 0) {
...
#else
  TIMER_MAKE_MEASUREMENTS(stencil_per_thread[statement_type][read_from_type][pointer_type][neighbor_index_type][fma_ins_type][prefetch_index_type][prefetch_distance][rb_size](rank), results, rank, NUM_TRIALS);
  barrier_wait(&my_barrier, rank);
  if (rank == 0) {
...
int main(int argc, char *argv[]) {
  ...
  ranks = (int32_t **) malloc(numThreads * sizeof(int32_t *));
  ...
  // run stencil tests
  for (t=0; t < numThreads; t++) {
    ranks[t] = (int32_t *) malloc(sizeof(int32_t));
    *ranks[t] = t;
  }
