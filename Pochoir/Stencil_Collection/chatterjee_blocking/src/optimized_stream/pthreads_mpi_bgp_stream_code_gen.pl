...
@prefetch_distances = (-1, 0, 64, 128, 256);
...
$specified_data_script = ("specified_bgp_data_generator.pl");
...
$greedy_data_script = ("greedy_bgp_data_generator.pl");
...
sub singly_nested_loop_code {
	...
	if ($write_array_count == 0) {
		...
		print OUT ("    result" . $lu . " = __fpmul(result" . $lu);
		print OUT (", __lfpd(&(ReadArray" . $rac . "_local[i" . i2s(2*$lu) . "])));\n");
	    }
	}
	else {
	    if ($read_array_count == 1) {
		print OUT ("    result" . $lu . " = __lfpd(&(ReadArray0_local[i" . i2s(2*$lu) . "]));\n");
	    }
	    elsif ($read_array_count > 1) {
		for ($rac=1; $rac < $read_array_count; $rac++) {
		    print OUT ("    result" . $lu . " = __fpmul(");
		    if ($rac == 1) {
			print OUT ("__lfpd(&(ReadArray0_local[i" . i2s(2*$lu) . "]))");
		    }
		    else {
			print OUT ("result" . $lu);
		    }
    ...
    for (my $lu=0; $lu < $loop_unroll; $lu++) {
	for ($wac=0; $wac < $write_array_count; $wac++) {
	    print OUT ("    __stfpd(&WriteArray" . $wac . "_local[i" . i2s(2*$lu) . "], result" . $lu . ");\n");
	}
    }
    print OUT ("  }\n");
	...
	for (my $lu=0; $lu < $loop_unroll; $lu++) {
	    print OUT ("  __stfpd(&ReadArray0_local[" . (2*$lu) . "], result" . $lu . ");\n");
	}
    }
...
sub generate_single_stream_function {
	...
	for (my $lu=0; $lu < $loop_unroll; $lu++) {
	    print OUT ("  result" . $lu . " = __cmplx(1.0, 1.0);\n");
	}
	print OUT ("\n");
    }
...
sub generate_code_variant_file {
    ...
    $case_count = 0;
    foreach $loop_unroll (@loop_unrolls) {
	print OUT ("    case $case_count: strcat\(output_str, \"loop_unroll = $loop_unroll, case number, $case_count; \"\)\; break\;\n");
	$case_count++;
    }
    print OUT ("  }\n");
...
sub generate_per_thread_function {
    print OUT (per_thread_function_name_str() . " {\n");
    print OUT ("  uint32_t my_thread_num\;\n");
    print OUT ("#if !defined(DEBUG)\n");
    ...
    print OUT ("    strcat\(output_str, \"BANDWIDTH (IN GB/S, WHERE 1 GB/S = 10^9 B/S) \"\)\;\n");
...
sub generate_main_function {
    ...
    print OUT "  uint32_t **thread_nums\;\n";
    ...
    print OUT "  thread_nums = \(uint32_t **\) malloc\(numThreads * sizeof\(uint32_t *\)\);\n\n";
    print OUT "#if !defined\(DEBUG\)\n";
    print OUT "#if defined\(PAPI_ENABLED\)\n";
    ...
    print OUT "    thread_nums[t] = \(uint32_t *\) malloc\(sizeof\(uint32_t\)\);\n";
...
sub generate_specified_data_script {
    ...
    print OUT ("\t\t\tmy \$output_file = (\"./\" . \$platform . \"_\" . \$compiler . \"_specified_mpi_\" . \$read_array_count_value . \"r_\" . \$write_array_count_value . \"w_\" . \$mem_footprint_in_GB . \"GB_\" . \$numThreads . \"t.txt\");\n");
    ...
    print OUT ("\t\t\t\t\t\tsystem (\"qsub -n 64 -t 19 -q prod-devel \$exec_file \$numThreads \$read_array_count_value \$write_array_count_value \$memory_footprint_in_B \$threadPaddingLength \$loop_unroll_value \$prefetch_distance\")\;\n");
    print OUT ("\t\t\t\t\t}\n");
    print OUT ("\t\t\t\t}\n");
    print OUT ("\t\t\t}\n");
    print OUT ("\t\t}\n");
...
sub generate_greedy_data_script {
    ...
    print OUT ("\tforeach \$read_array_count_value (\@read_array_count_values) {\n");
    print OUT ("\t\tforeach \$write_array_count_value (\@write_array_count_values) {\n");
    print OUT ("\t\t\tmy \$output_file = (\"./\" . \$platform . \"_\" . \$compiler . \"_greedy_mpi_\" . \$read_array_count_value . \"r_\" . \$write_array_count_value . \"w_\" . \$mem_footprint_in_GB . \"GB_\" . \$numThreads . \"t.txt\");\n");
