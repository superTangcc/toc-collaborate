...
/* -----------------------------------------------------------------
 *  TIMER_MAKE_MEASUREMENTS( CODE, results, num_trials );
 *
 *  Input  : CODE, results, num_trials
 *  Output : results
 *
 ...
 */
#ifdef CYCLE_TIME
#define TIMER_MAKE_MEASUREMENTS( CODE, results, num_trials ) { \
		uint32_t _i; \
                uint64_t _t_start, _t_end; \
		for ( _i = 0; _i < num_trials; _i++ ) { \
                        init_grids(); \
