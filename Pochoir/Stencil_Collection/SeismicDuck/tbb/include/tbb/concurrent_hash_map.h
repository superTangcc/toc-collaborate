    ...
    public:
        ...
        void mark_rehashed_levels( hashcode_t h ) throw () {
            ...
            while( segment_ptr_t seg = my_table[++s] )
                if( seg[h].node_list == internal::rehash_req ) {
                    seg[h].node_list = internal::empty_rehashed;
...
template<typename Key, typename T, typename HashCompare, typename A>
void concurrent_hash_map<Key,T,HashCompare,A>::rehash(size_type n) {
    reserve( n ); // TODO: add reduction of number of buckets as well
    ...
    for(; b <= mask; b++, bp++ ) {
        ...
        if( n == internal::rehash_req ) { // rehash bucket, conditional because rehashing of a previous bucket may affect this one
            ...
            for( node_base **p = &b_old->node_list, *n = *p; is_valid(n); n = *p ) {
                hashcode_t c = my_hash_compare.hash( static_cast<node*>(n)->item.first );
...
#if TBB_USE_PERFORMANCE_WARNINGS
    int size = int(my_size), buckets = int(mask)+1, empty_buckets = 0, overpopulated_buckets = 0; // usage statistics
