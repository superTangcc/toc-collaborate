...
public:
	GeologySliderDialog() : SliderDialog("GEOLOGY_SLIDERS") {
...
public:
	ShotSliderDialog() : SliderDialog("SHOT_SLIDERS") {
...
public:
	ColorSliderDialog() : SliderDialog("COLOR_SLIDERS") {
		setLimits(0,0,2);
	}
	void setValues() {
...
static int FrameIntervalLimitTag=1;

class SpeedDialog: public SliderDialog {
public:
	SpeedDialog() : SliderDialog("SPEED_SLIDERS") {
		setLimits(0,1,5);
		setLimits(1,0,2);
		oldIntervalLimit=1;
	}
...
static MessageDialog TheAboutTheAuthorDialog("ABOUT_THE_AUTHOR");
static MessageDialog TheKeyboardHelpDialog("KEYBOARD");

static Font TheFont("FONT_SANS_16");
...
static WheelMeter CashMeter("CASH_METER");
static WheelMeter LevelMeter("LEVEL_METER");
static BarMeter WaterMeter("WATER_METER");
static BarMeter OilMeter("OIL_METER");
static BarMeter GasMeter("GAS_METER");
static RubberImage PanelBackground("PANEL");
static DigitalMeter FrameRateMeter(6,1);

static MessageDialog TheLevelContinueDialog("LEVEL_CONTINUE_DIALOG");
static MessageDialog WarnBreakDrillDialog("WARN_BREAK_DRILL_DIALOG");
static MessageDialog WarnAwayFromCultureDialog("WARN_AWAY_FROM_CULTURE_DIALOG");
...
public:
	...
	bool isPeekAllowed() const {
		return state==gsTraining||state==gsBonus||gsOver;
	}
...
static Sprite LandRig("LAND_RIG");
static Sprite FloatingRig("FLOATING_RIG");
static Sprite* OilRig = &LandRig;
static int OilRigVerticalOffset;
static AnimatedSprite DrillBit("DRILL_BIT",9);
...
static void CreateNewArea( bool recycle=false ) {
	...
	if( recycle ) {
		...
		for( int trial=0; trial<3; ++trial ) {
			...
			float figureOfMerit = (s.volume[GAS]+s.volume[OIL])*s.numTrap;
