...
#	generate a bibtex file with name: latexfile.bib)
#
#Bugs: This script cannot handle bib labels containing a "+" and there must be no blank lines inside a bib entry.
		...
		foreach $line (@row){
			...
			if (($found<2)){
				...
				#	print "\% Started\n";<STDIN>;
					...
					# Since allpapers.bib is generated from bibdesk, a blank line means the end.
					if ( ($line=~/^$/)) {
					    $found=2;
					}
				}
